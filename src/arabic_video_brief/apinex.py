...
3. `post_title_ar`: A direct, faithful, and accurate translation of the source video's original title (from VIDEO CONTEXT) into 100% pure Modern Standard Arabic. Faithfully translate the core meaning and accurately transliterate any foreign guest or channel names phonetically into Arabic letters (e.g. "How Emotions Are Made - Lisa Feldman Barrett" -> "كيف تُصنع المشاعر – ليزا فيلدمان باريت"). Strictly no English words, no emojis, and no hashtags.
...
def _call_apinex_api(
    ...
    for attempt in range(5):
        try:
            with urllib.request.urlopen(req, timeout=timeout) as response:
                ...
                if not choices:
                    raise RuntimeError(f"APInex returned empty choices: {result}")
                return choices[0]["message"]["content"]
        except urllib.error.HTTPError as err:
            body = err.read().decode("utf-8", errors="ignore")
            last_err = RuntimeError(f"APInex API HTTP {err.code} Error ({err.reason}): {body}")
            if err.code in (429, 500, 502, 503, 504) and attempt < 4:
                time.sleep(4.0 * (attempt + 1))
                continue
            raise last_err from err
        except Exception as err:
            last_err = RuntimeError(f"APInex API connection failed: {err}")
            if attempt < 4:
    ...
    if last_err:
        raise last_err
    raise RuntimeError("APInex API failed after retries")


def _adjust_summary_apinex(
    base_url: str,
    api_key: str,
    model: str,
    current_summary: str,
...
def analyze_video_apinex(
    source: str,
    local_path: Path,
    settings: Settings,
    metadata: dict[str, Any] | None = None,
) -> dict[str, Any]:
    api_key = settings.apinex_api_key
    if not api_key:
        raise RuntimeError("APINEX_API_KEY is not configured in .env")

    model = settings.apinex_model or "free/gemini-3.8-flash"
...
{transcript[:8000]}
""")
        # Compact fallback if first attempt encounters payload/timeout limits
        contexts_to_try.append(f"""VIDEO CONTEXT (GROUND TRUTH):
- Original Video Title: {title}
- Channel / Speaker: {channel}
- Video Description:
...
""")
    ...
    if not content:
        if last_error:
            raise last_error
        raise RuntimeError("APInex failed to return a response")
    ...
    if not isinstance(payload, dict) or "summary_ar" not in payload:
        raise RuntimeError(f"APInex JSON missing required fields. Output was:\n{content[:500]}")
    ...
    if not (settings.summary_min_words <= count <= settings.summary_max_words):
        try:
            time.sleep(2)
            adjusted = _adjust_summary_apinex(
                settings.apinex_base_url,
                api_key,
                model,
                summary,
